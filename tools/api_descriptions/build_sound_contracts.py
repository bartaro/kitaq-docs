...
vb_format=msg('vb_format','直接参照する曲は固定ROMなどISRが常に読める場所に置きます。レコードはdelay,CH2,CH1,CH3,CH4の5バイトです。ノートの0xFFは保持、0xFDは停止です。delay位置の0xFFは終端、0xFEはループです。WRAMキューは別モードで、補充処理は呼び出し側が実装します。','Keep direct song data in fixed ROM or another area always visible to the ISR. A record is five bytes: delay, CH2, CH1, CH3, CH4. In note fields, 0xFF holds the current voice and 0xFD stops it. In the delay field, 0xFF ends playback and 0xFE loops. WRAM queue playback is a separate mode; the caller implements refilling.')
...
('ch','物理チャンネル番号：0=CH1、1=CH2、2=CH3、3=CH4。曲ストリームの番号とは区別します。','Physical channel: 0=CH1, 1=CH2, 2=CH3, 3=CH4. These differ from music-stream channel IDs.'),
...
modules['gb:audio']=[msg('module_audio','4音源の直接発音、バンク付き音楽・効果音、左右定位、音量フェード、独自波形を扱います。音楽ストリームのチャンネル番号はCH1=0・CH2=1・CH4=2・CH3=3で、物理チャンネル番号とCH3/CH4が逆です。','Provides direct access to four voices, banked music/effects, stereo routing, volume fades and custom waves. Music-stream channel IDs are CH1=0, CH2=1, CH4=2, CH3=3; CH3/CH4 are reversed relative to physical channel IDs.'),gb_setup,sfx_format]
