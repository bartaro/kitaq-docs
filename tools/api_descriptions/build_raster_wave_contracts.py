...
add('RunFrame','VBlankから次の画面の144ラインを描画し、各ラインの背景横位置を更新します。描画後に分周カウンターを進め、指定間隔で位相を更新します。','Wait for VBlank and render the following 144 visible lines with per-line background X. Afterwards advance the divider counter and update phase at the selected interval.',
    ...
    notes=[text('lcd','LCDがオフなら、状態を変えず直ちに戻ります。LCDがオンの場合は描画終了まで戻りません。割り込みの許可状態を復元し、最後のSCXをScrollヘルパーの追跡値へ反映します。','If the LCD is off, return immediately without changing the effect. Otherwise block until visible drawing finishes, restore the interrupt-enable state and synchronize the Scroll helper with the final SCX.')])
