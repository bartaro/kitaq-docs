...
def overview(value,language):
    ...
    block+='<p>'+('KUROSAKIで3サイズ×3種類の画像を照合し、線・立方体・消去の9ケースを確認しました。実機での計測結果ではありません。' if language=='ja' else 'KUROSAKI checked lines, a cube and clearing at each size: nine image checks in total. These are emulator checks, not hardware measurements.')+'</p></section>'
    value=re.sub(r'<!-- fc-wire-guide:start -->.*?<!-- fc-wire-guide:end -->','',value,flags=re.S)
    block='<!-- fc-wire-guide:start -->'+block+'<!-- fc-wire-guide:end -->'
