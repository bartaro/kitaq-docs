"""Run identical language fixtures on both compilers and inspect emulator RAM.
    ...
    for platform in ([a.platform] if a.platform else ['gb', 'fc']):
        ...
        for fixture in fixtures:
            ...
            for variant, flags in variants:
                ...
                cmd = [str(compiler), str(source), '-o', str(rom), '--no-cache', '--no-disasm'] + flags
                ...
                if build.returncode:
                    ...
                    for mode in (['dmg','cgb'] if platform=='gb' else ['ntsc']):
                        ...
                        if platform=='gb':
                            ...
                            cmd=[str(emulator),'run',str(rom),'--frames','30','--headless','--snapshot',str(state)]
