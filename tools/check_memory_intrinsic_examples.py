"""Run the RAM-copy teaching ROMs and check every displayed result and ink color."""
...
OUTPUT=SITE/'verification/api-memory-intrinsics'
...
for platform,mode in [('gb','dmg'),('gb','cgb'),('fc','nrom')]:
    ...
    source=SITE/'samples/api-examples'/platform/'memory_intrinsics.c'
    ...
    supports=['samples/memory_example_checks.h']+(['samples/gb_tile_color_example.h','samples/gb_tile_example.h','samples/gb_common.h','samples/font_gb.h'] if platform=='gb' else ['samples/fc_common.h','samples/font.chr'])
    ...
    if process.returncode==0:
        ...
        if process.returncode==0:
            ...
            labels=[(1,0,'RAM COPY AND FILL')]
            for y,text,value in [(2,'COPY WORD','257'),(4,'FILL WORD','257'),(6,'COPY BYTE','255'),(8,'FILL BYTE','255'),(10,'COPY16','016'),(12,'COPY32','032'),(14,'ZERO CALLS','002'),(16,'FAILED','000')]:
