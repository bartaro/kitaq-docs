...
## 本書の版と検証の読み方
本書は**2026年9月14日のソース**に基づいています。参照一覧にはソースと実行ファイルのSHA-256を記録しています。各試験の入力、条件、確認範囲は検証記録を参照してください。
...
'kitaqgb':r'''
## 1　KITAQGBを知る
KITAQGBはZachtronicsのNORCALをフォークして発展した、GB/CGB用のC系コンパイラです。

{{ORIGIN}}

Cファイルを読み、CPU命令へ変換し、ゲーム機が読み込むROMにまとめます。一般のPC用Cコンパイラとは言語・標準ライブラリ・呼び出し規約が異なります。
...
### 式の結果と評価回数
比較演算子 `==`、`!=`、`<`、`<=`、`>`、`>=` と論理演算子 `!`、`&&`、`||` は、偽なら0、真なら1を返します。結果を `u16` 変数へ保存したり、引数・戻り値・算術式に使えます。例えば `score = 500 + (lives != 0);` は、残機があれば501、なければ500になります。

`&&` は左辺が0なら右辺を評価せず、`||` は左辺が0以外なら右辺を評価しません。`pointer != 0 && pointer->active != 0` では、ポインタが0のとき右辺へアクセスしません。16ビット値は上位バイトも含めて判定するため、256も真です。ビット演算子 `&`、`|` には短絡評価がありません。

`++value` は更新後、`value++` は更新前の値を返します。配列要素、ポインタの参照先、構造体メンバーにも使えます。`buffer[index()]++` は `index()` を1回呼びます。`u16 *p` に対する `p++` は次の要素へ2バイト進み、`(*p)++` は指している値を増やします。

`sizeof(array)` は配列全体のバイト数、`sizeof(pointer)` は2です。例えば `u16 values[9];` なら `sizeof(values)` は18です。ROM配列・ローカル配列・構造体内の配列も対象になります。`sizeof(function())` は戻り値の型の大きさを調べるだけで、関数を呼びません。

関数宣言と定義では、引数の型と順番を合わせます。引数名は一致しなくてもよく、関数本体では定義側の名前を使います。例えば `u8 next(u8 input);` に対して `u8 next(u8 value) { return (u8)(value + 1); }` と定義できます。引数やローカル変数は同名のグローバル変数を隠します。

`?:`で配列や文字列を選ぶと、結果は選んだ要素型へのポインタになります。`show(ready ? "READY" : "WAIT");`のように直接引数へ渡せます。`u16`配列`a`と`b`を選ぶ`(ready ? a : b) + 1`は、選択先の2番目の要素へ2バイト進みます。配列そのものをコピーする操作ではありません。

`condition ? yes : no` は条件を評価してから、選ばれた側だけを評価します。条件や選択肢には変数を使ったシフト式も書けます。例えば `on = (pattern & (0x80 >> bit)) != 0 ? 4 : 2;` は対象ビットに応じて4か2を選びます。`&&`、`||`で飛ばされる右辺の関数呼び出しも実行されません。ループ内の`continue`では、`for`の更新式を1回実行してから条件を再評価します。`while`・`do ... while`では条件へ進みます。
...
標準のLegacy ABIは引数やローカル領域を固定配置する設計です。再帰や割り込みによる再入をPC用Cと同様に扱わないでください。`__stackcall` や `--abi=stack` は呼び出し規約に関わる上級指定です。混在するときはビルドのABIレポートと実行を確認します。関数ポインター経由のコールバック呼び出しは`--abi=stack`では未対応です。システムやシーンのコールバックを使う完成サンプルは、掲載したビルド例どおり標準のLegacy ABIを使います。
...
各API辞典には宣言、所属ヘッダー、実装ソース、呼び出し例を掲載しています。引数を受け渡すコード断片では、使うバッファやオブジェクトを呼び出し側で準備してください。ROMとしてビルドできる教材は「完全なサンプルプログラム」欄にあります。

## 2　1フレームの組み立て
`system_init` はフレーム状態を初期化してNMIを有効にします。`system_wait_vblank` はNMIを待ち、ソフト側のフレーム数を増やしてから、登録したコールバックを同期的に1回呼びます。`system_set_vblank_callback(0)` で解除できます。コールバックはNMIハンドラーの中ではなく、待機関数を呼んだ側で実行します。
...
音楽ストリームの `AUDIO_CMD_NOTE` / `AUDIO_CMD_SET_INST` / `AUDIO_CMD_STOP_CHANNEL` と通常APIは、共通の番号 **0=CH1、1=CH2、2=CH3、3=CH4** を使います。CH1はスイープ付きパルス音、CH2はパルス音、CH3は波形メモリ、CH4はノイズです。ノート上限はヘッダーの `AUDIO_NOTE_MAX=67` です。
...
## 8　VBlank IRQ音楽
`audio_vblank.c` は専用の再生形式を使います。時間を指定するレコードは `delay, ch1_note, ch2_note, ch3_note, ch4_noise_param` の5バイトです。直接参照できる曲を読む方式と、WRAMキューから読み出す方式があります。キュー方式ではAudioVBlank_QueueResetで初期化し、AudioVBlank_QueueRefillで5バイト単位のレコードを補充してからAudioVBlank_QueuePlayで開始します。補充は前景処理の1か所から行います。`LOOP` は直接参照方式だけ、`IMMEDIATE` はキュー方式だけで認識されます。通常の `audio.c` のストリームはそのまま渡せません。
...
`physics2d` は矩形、`physics2d_circle` は円、`physics3d` は3DのAABBを扱います。利用側でworldとbodyの配列を用意して初期化し、速度や重力を設定してstepを呼びます。位置と1ステップ当たりの速度は単位を統一してください。ライブラリによる暗黙の画素換算はありません。円の例は位置40・速度2を使います。逆質量0は固定物体です。係数の表現と途中計算の範囲は各ヘッダーを確認してください。特に `kq2d_body_apply_friction` は係数を符号付き8ビットに変換するため、128～255は負数になります。通常の符号なしQ8の減衰係数としては扱えません。1ステップのプログラム例は `gb_circle.c` です。

`wire3d_dmg` は、ゲームボーイ向けのモノクロ・ワイヤーフレーム描画ライブラリです。128×96は `wire3d_dmg_96.c`、128×120は `wire3d_dmg.c` をビルドし、`Wire3DDMG_*` を使います。`wire3d` と `dmg3d` も、それぞれ96行と120行の設定を選ぶ入口として使えます。1つのプログラムでは入口を1つだけビルドしてください。`wire3d_cgb` はカラー専用の描画ライブラリです。それぞれが使用するRAM・VRAM・画面領域を確保してください。白黒の2方式は、奥行きの範囲をまたぐ辺を切り詰めずに省略します。シーンの隠線処理も、面の外接矩形と線上の5点を使う近似であり、画素ごとの奥行き判定ではありません。 CGB版は倍速とDMAを使用し、`--cgb=cgb_only` が必要です。

`Wire3DDMG_BeginFrame` (`WIRE3D_DMG_HEIGHT=96`) は描画バッファを消去します。一方、`Wire3DDMG_BeginFrame` (`WIRE3D_DMG_HEIGHT=120`) は隠線判定の状態だけを戻し、描画データはVRAMへ転送したときに消去されます。差分転送では前フレームのタイルも処理して古い画素を消します。補助転送は主バッファの一部を共有しており、独立したバッファではありません。採用する方式に合ったフレーム処理順にしてください。

CGB版の線には色番号1・2・3を使います。通常の128×96描画では色のビットを重ねるため、色1と色2が交差すると色3になります。色0で線を消すことはできません。フレームを消去するか、専用の消去関数を使ってください。通常の `Wire3DCGB_DrawLine2D` とモデル描画は差分転送の範囲を記録しません。線の範囲も記録する場合は `Wire3DCGB_DrawLineClipped2D` を使い、画面全体を次の差分転送に含める場合は `Wire3DCGB_InvalidateFrameHistory` を呼びます。

160×144モードでは、1フレームに割り当てられるタイルは最大127枚です。高速な線描画でタイルの割り当てに失敗したり、画面外の座標を指定したりすると、`Wire3DCGB_GetFullScreenOverflow()` が非ゼロとなり、次のフレーム初期化まで画素の書き込みを停止します。頂点は選択した画面内に収めてください。三角形マスクの余白の右端は128×96ではX=127、全画面ではX=159までです。特に全画面描画とFastMapでは、APIの説明にあるWRAMバンク設定を守ってください。

## 10　ゲーム状態・オブジェクト・弾幕
`scene` はタイトル・ゲーム・ポーズ等の切り替え、`entity` は固定数のオブジェクトプール、`chain` は関節追従と座標履歴を扱います。蛇やひもの各節を動かすには `ChainBody` を使います。プールの生成失敗は0xFFなどの戻り値で確認し、成功を確認してから `entity_get` の指す先を使います。
...
構造体のコピー、通常の関数呼び出し、for、while、do-whileを使用できます。do-whileは本体を少なくとも1回実行してから条件を判定します。continueは末尾の条件判定へ進み、breakはループを抜けます。switchでは0～255のcase定数で分岐し、caseに一致しなければdefaultを実行します。判定式は1回だけ評価します。breakは最も内側のループまたはswitchを抜け、switch内のcontinueは外側のループへ進みます。
...
ROM配列を通常の添字やCポインターで参照すると、その配列は共通バンク0へ配置されます。ただし、`#pragma fixed_bank` で明示した配置は保持します。これにより、呼び出し元が切替バンクで動いていてもデータを参照できます。明示的なfarアクセスには、`__farpeek8(__bankof(table), table)` のように、同じ呼び出しの引数で配列名とその配列の `__bankof` を組み合わせます。バンク番号を問い合わせるだけでは共通バンクへの配置を要求しません。fixed_bankを指定したデータやアセンブリ中の参照は、呼び出し側で正しいマッピングを維持してください。この判定は構文に基づく配置処理で、ポインターの流れ全体を解析するものではありません。共通バンクの容量制限も適用されます。
...
`ppu.h` と `ppu.c` は画面のオン・オフ、32バイトのパレット転送、ネームテーブル全体の初期化を提供します。`nes_ppu_seek_bytes(hi,lo)` はアドレスラッチをリセットして2バイトのアドレスを設定します。`runtime.c` の16ビット引数の `nes_ppu_seek(address)` と併用できます。転送・初期化は描画を止めて実行してください。
...
## 7　シーン・アクター・エンティティ
`actor` / `entity` は固定配列のゲームオブジェクト、`scene` は場面の状態を扱います。配列の上限を超えた生成を検出し、破棄したIDを使い続けないでください。シーンの切り替え・更新・描画は、それぞれに登録したコールバックを同期的に呼び出します。各APIの呼び出し順と再入制約を確認してください。

`chain` は `ChainBody` による関節追従と `Chain` による座標履歴を提供し、`collision` は矩形などの接触を判定します。動かす、衝突を調べる、描く、の順序を揃えると1フレーム遅れの判定を避けられます。

## 8　数学と物理
`fixed.h` はQ8.8演算、`math_fast` / `math_fixed` は数値処理、`math_lut` は表による計算です。`physics2d` は矩形bodyの積分・重力・AABB衝突・面応答、`physics3d` は回転しない箱の3D衝突・反発・衝撃値を扱います。worldとbody配列を用意し、初期化、設定、stepの順に使います。座標と速度は呼び出し側で統一した整数単位です。
...
'kokura':r'''
## 1　KOKURAとは
KOKURAはGB/CGB用のエミュレータと観測ツールです。ROMを動かすだけでなく、画面、音、CPU、メモリ、バンク、入力、診断イベントを記録します。コマンドラインの実行ファイルは `kokura-cli.exe` です。
...
# Queue capacity queries (source-reviewed API update).
TEXT['gb-library'] += '\n\n`vram_get_queue_capacity()` はキューの総項目数（標準32項目）、`vram_get_queue_free()` は未使用の項目数、`vram_get_queue_used()` は使用中の項目数を返します。転送量にかかわらず、予約した処理1件で1項目を使用します。いずれも転送キューの容量であり、実際のVRAMの未使用領域を調べる関数ではありません。\n'
TEXT['fc-library'] += '\n\n`vram_get_queue_capacity()` はコマンドバッファの総容量（128バイト）、`vram_get_queue_free()` は総容量から `vram_get_queue_used()` を引いた空きバイト数を返します。アドレスなどの管理情報を含むコマンドのサイズであり、実際のVRAMの空き容量ではありません。タイル1個の書き込みには4バイト、同じ値での埋め込みには5バイト、ポインターを使ったコピーには6バイト必要です。commit後はNMIでキューが消費されるため、空きの確認はcommit前に行ってください。\n'
