...
## 本書の版と検証の読み方
**2026年9月14日のローカルソースを基準** とする版です。「最新」はこの採取時点を指し、将来のGitHub公開版の更新まで自動追従する意味ではありません。参照ソースと使用実行ファイルのSHA-256を同梱しています。過去のREADMEにある「未対応」「確認済み」は、そのまま現在の保証として転載していません。
...
`physics2d` は矩形、`physics2d_circle` は円、`physics3d` は3DのAABBを扱います。利用側でworldとbodyの配列を用意して初期化し、速度や重力を設定してstepを呼びます。位置と1ステップ当たりの速度は単位を統一してください。ライブラリによる暗黙の画素換算はありません。円の例は位置40・速度2を使います。逆質量0は固定物体です。係数の表現と途中計算の範囲は各ヘッダーを確認してください。特に旧来の `kq2d_body_apply_friction` は係数を符号付き8ビットに変換するため、128～255は負数になります。通常の符号なしQ8の減衰係数としては扱えません。1ステップのプログラム例は `gb_circle.c` です。

`wire3d_dmg` は、ゲームボーイ向けのモノクロ・ワイヤーフレーム描画ライブラリです。128×96は `wire3d_dmg_96.c`、128×120は `wire3d_dmg.c` をビルドし、`Wire3DDMG_*` を使います。旧 `wire3d` と `dmg3d` は、それぞれの設定を選ぶ互換用の入口です。同時に複数の入口をビルドしないでください。カラー専用版 `wire3d_cgb` の名前は変更していません。それぞれが使用するRAM・VRAM・画面領域を確保してください。白黒の2方式は、奥行きの範囲をまたぐ辺を切り詰めずに省略します。シーンの隠線処理も、面の外接矩形と線上の5点を使う近似であり、画素ごとの奥行き判定ではありません。 CGB版は倍速とDMAを使用し、`--cgb=cgb_only` が必要です。
