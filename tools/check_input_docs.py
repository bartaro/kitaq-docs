...
def main():
    ...
    parser.add_argument('--suite',choices=['input','padrepeat','expansion','vram','vram-memory','vramq','cgb-palette','cgb-dma-wram','asset','bank','sprite','oam','fc-oam','oam-library','vram-macros','runtime-queue'],default='input');args=parser.parse_args()
    ...
    assert len(results)==len(inputs)*9 and len(inputs)=={'input':35,'padrepeat':6,'expansion':6,'vram':30,'vram-memory':9,'vramq':10,'cgb-palette':28,'cgb-dma-wram':8,'asset':17,'bank':32,'sprite':28,'oam':1,'fc-oam':9,'oam-library':8,'vram-macros':5,'runtime-queue':4}[args.suite], len(results)
    modules=json.loads((api_contracts.SOURCE/(args.suite+'_modules.json')).read_text(encoding='utf-8')) if args.suite in ['input','vram','cgb-palette','asset','bank','sprite','oam-library','runtime-queue'] else {}
    module_count=0
    for index,language in enumerate(api_contracts.ORDER):
        folder=SITE if language=='ja' else SITE/language
