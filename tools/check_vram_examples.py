...
for platform,mode in [('gb','dmg'),('gb','cgb'),('fc','nrom')]:
    ...
    support = ['samples/gb_tile_example.h','samples/gb_common.h','samples/font_gb.h'] if platform=='gb' else ['samples/fc_common.h','samples/api-examples/fc/vram_shapes.chr']
    ...
    if process.returncode==0:
        ...
        if process.returncode==0 and image.exists():
            ...
            counts=check_pixels(image,labels,platform);shapes=geometry_errors(image)
            row.update(image=image.relative_to(SITE).as_posix(),image_sha256=sha(image),expected_labels=labels,label_pixel_mismatches=counts,geometry_pixel_mismatches=shapes,frames=frames,passed=counts==0 and shapes==0 and frames==180)
