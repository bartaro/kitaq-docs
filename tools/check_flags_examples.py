"""Run the flag and quest teaching ROMs and check every displayed result and ink color."""
...
OUTPUT=SITE/'verification/api-flags'
...
for platform,mode in [('gb','dmg'),('gb','cgb')]:
    ...
    source=SITE/'samples/api-examples'/platform/'flags_quests.c'
    ...
    command[2:2]=[str(library/'flags.c')]
    ...
    supports=[]+(['samples/gb_tile_color_example.h','samples/gb_tile_example.h','samples/gb_common.h','samples/font_gb.h'] if platform=='gb' else ['samples/fc_common.h','samples/font.chr'])
    ...
    row['library_sha256']=sha(library/'flags.c')
    if process.returncode==0:
        ...
        if process.returncode==0:
            ...
            labels=[(1,0,'FLAGS AND QUESTS')]
            for y,text,value in [(2,'BEFORE','000'),(4,'DOOR OPEN','001'),(6,'NEIGHBOR','000'),(8,'DOOR CLOSED','000'),(10,'ACCEPTED','001'),(12,'COMPLETED','003'),(14,'LAST QUEST','255'),(16,'FAILED','000')]:
                labels.extend([(1,y,text),(16,y,value)])
