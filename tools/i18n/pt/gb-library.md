...
Cada entrada de API identifica a declaração, o cabeçalho, a implementação e um exemplo de uso. As funções sem exemplo anterior receberam um novo fragmento que demonstra a passagem de argumentos. O código que o chama deve fornecer os buffers e objetos necessários. Na seção de programas completos, você encontra exemplos que podem ser compilados diretamente como ROMs.
...
Os comandos de fluxo musical `AUDIO_CMD_NOTE` e `AUDIO_CMD_SET_INST` mantêm uma ordem histórica: **0=CH1, 1=CH2, 2=CH4, 3=CH3**. Não a confunda com as constantes de canal da API comum. O cabeçalho define atualmente `AUDIO_NOTE_MAX=67`.
