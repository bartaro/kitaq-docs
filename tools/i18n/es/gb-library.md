...
Cada entrada de la API indica la declaración, el archivo de cabecera, la implementación y un ejemplo de uso. Para las funciones que no tenían ejemplo se ha añadido un fragmento que muestra cómo pasar los argumentos. El código que lo invoque debe preparar los búferes y objetos necesarios. La sección de programas completos contiene ejemplos que sí se pueden compilar directamente como ROM.
...
Las órdenes del flujo musical `AUDIO_CMD_NOTE` y `AUDIO_CMD_SET_INST` conservan un orden histórico: **0=CH1, 1=CH2, 2=CH4, 3=CH3**. No lo confundas con las constantes de canal de la API habitual. La cabecera define actualmente `AUDIO_NOTE_MAX=67`.
