"""Build the FC intrinsic queue sample and check geometry, colors and queue assertions."""
...
OUTPUT=SITE/'verification/api-vramq'
...
source=SITE/'samples/api-examples/fc/vramq_intrinsics.c'
...
row={'platform':'fc','mode':'nrom','source':source.relative_to(SITE).as_posix(),'source_sha256':sha(source),'compiler_sha256':sha(compiler),'build_command':command,'build_exit':process.returncode,'passed':False,
...
if process.returncode==0:
 ...
 if process.returncode==0 and image.exists():
  ...
  labels=[(1,0,'VRAMQ INTRINSICS'),(1,10,'BLUE COPY / GREEN FILL'),(1,19,'RED SENTINEL / NMI / PUT')]
