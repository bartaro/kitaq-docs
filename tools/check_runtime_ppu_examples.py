...
OUTPUT=SITE/'verification/api-runtime-ppu'
...
records=[]
for mode,length in [('runtime',256),('pair',255)]:
 folder=OUTPUT/mode;folder.mkdir(parents=True,exist_ok=True)
 source=SITE/('samples/api-examples/fc/'+('runtime_ppu.c' if mode=='runtime' else 'ppu_address_pair.c'))
 ...
 row={'platform':'fc','mode':mode,'source':source.relative_to(SITE).as_posix(),'source_sha256':sha(source),'compiler_sha256':sha(compiler),'library_sha256':{name:sha(REPOS/'kitaqfc/lib'/name) for name in ['intrinsics.h','runtime.h','runtime.c','ppu.c']},'build_command':command,'build_exit':process.returncode,'passed':False,
...
(OUTPUT/'results.json').write_text(json.dumps({'records':records,'scope':'Direct stream lengths 0/255/256, copied patterns, fill, both address-latch signatures and wait completion. NROM emulator only; real hardware not tested.'},indent=2),encoding='utf-8')
