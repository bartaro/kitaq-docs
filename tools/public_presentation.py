...
def public_file(relative):
    ...
    if len(path.parts)>2 and path.parts[0]=='verification' and path.parts[1] in ('api-memory-intrinsics','api-bit-intrinsics','api-rng') and path.parts[2] in ('gb','fc'):
        return False
    if path.parts[:3] in [('verification','api-flags','state'),('verification','api-rle','state'),('verification','api-text-layout','state'),('verification','api-dialogue','state'),('verification','api-batch100','state')]:
        return False
