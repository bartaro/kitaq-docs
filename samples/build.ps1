...
if (!$GbCompiler) { $GbCompiler = Join-Path $Root 'kitaqgb\kitaqgb.exe' }
if (!$FcCompiler) { $FcCompiler = Join-Path $Root 'kitaqfc\kitaqfc.exe' }
...
foreach ($program in $programs) {
    ...
    $lib = Join-Path $Root $(if ($isGb) { 'kitaqgb\lib' } else { 'kitaqfc\lib' })
    $compileArgs = @()
