// Run the same ROM in four DMG-07 sessions. Every console discovers its own slot.
...
while(1){
    ...
    if(LinkDmg07_HasPacket()){
        // Snapshot the first published packet before the next byte begins.
...
m_wait();M_LCDC=0;m_text(1,0,"DMG07 EXCHANGE");
