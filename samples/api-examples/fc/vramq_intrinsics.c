// Learn intrinsic queue records, independent reset flags and 0/255-byte transfers.
#include "fc_common.h"
...
void main() {
    ...
    failures=0;
    __vramq_clear(); __vramq_clear_overflow();
    ...
    i=0;
    while (i<32) { __vramq_put(0x229F,0); i++; }
    if (__vramq_len()!=128 || __vramq_overflow()!=0) failures++;
    __vramq_put(0x229F,1);
    ...
    __vramq_put(0x229F,0);
    // example:__vramq_clear:start
    __vramq_clear();
    if (__vramq_len()!=0 || __vramq_overflow()!=1) failures++;
    // Clearing queued work deliberately leaves the overflow latch set.
    // example:__vramq_clear:end
    __vramq_put(0x229F,0);
    ...
    __vramq_commit(); __vramq_clear();
    __vramq_put(0x229F,1); __vramq_exec();
    if (__vramq_len()!=4) failures++;
    __vramq_clear();

    i=0;
    while (i<255) { source_bytes[i]=(u8)(i%3+1); i++; }
    // example:__vramq_put:start
    __vramq_put(0x2282,1); // Red 8x8 sentinel at pixel (16,160).
    // example:__vramq_put:end
    // Zero-length records still consume 6 and 5 bytes, but preserve the sentinel.
    __vramq_copy(0x2282,source_bytes,0);
    __vramq_fill(0x2282,0,0);
    // example:__vramq_copy:start
    __vramq_copy(0x2040,source_bytes,255);
    source_bytes[0]=3; // The executor must see this change through its retained pointer.
    // example:__vramq_copy:end
    fill_value=1;
    // example:__vramq_fill:start
    ...
    // example:__vramq_fill:end
    __vramq_put(0x229E,1); // Red solid tile at pixel (240,160).
    ...
    __vramq_put(0x229C,2); // Red left-half tile at pixel (224,160).
    __vramq_commit(); __nmi_wait(); __scroll_set(0,0);
    if (__vramq_len()!=0) failures++;
    m_text(1,0,"VRAMQ INTRINSICS"); m_wait();
