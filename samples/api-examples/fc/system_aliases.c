...
__location(0x4017) u8 frame_irq;__location(0x4010) u8 dmc_irq;u8 frame_calls;u16 frame_seen;u8 events;u8 event_log[16];void on_frame(){frame_calls++;frame_seen=fc_frame_count();}void event(u8 n){event_log[events]=n;events++;}void enter0(){event(10);}void update0(){event(11);}void draw0(){event(12);}void exit0(){event(13);}void enter1(){event(20);}void update1(){event(21);}void draw1(){event(22);}void exit1(){event(23);}SceneDef scenes[2];
...
// example:fc_init:start
fc_init();result[0]=fc_frame_count();
// example:fc_init:end
// example:fc_set_vblank_callback:start
fc_set_vblank_callback(on_frame);
// example:fc_set_vblank_callback:end
// example:fc_wait_vblank:start
fc_wait_vblank();fc_wait_vblank();fc_wait_vblank();result[1]=frame_calls;result[2]=frame_seen;
// example:fc_wait_vblank:end
// example:fc_frame_count:start
result[3]=fc_frame_count();
// example:fc_frame_count:end
...
fc_set_vblank_callback(0);fc_wait_vblank();result[5]=frame_calls;result[6]=fc_frame_count();kq_system_frame=65535;fc_wait_vblank();result[7]=fc_frame_count();result[8]=system_get_frame8();
