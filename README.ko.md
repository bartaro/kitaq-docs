...
7권의 본편을 9개 언어로 제공합니다. 소스 설명은 2026년 9월 14일 기준으로 갱신했으며, 모든 언어판에 같은 API 항목 1,053개와 완성 예제 프로그램 47개를 수록했습니다. 로컬에서는 한국어 `ko/index.html`, 영어 `en/index.html`, 일본어 `index.html`을 열면 됩니다. 각 권에서 언어를 바꿀 수 있습니다. 과거의 빌드·실행 기록은 당시 버전에 대한 결과이며, 이번 소스 갱신으로 재검증된 것은 아닙니다.
...
소스 수집과 빌드에는 원본 소스 트리와 각 도구가 필요합니다. 글꼴 변환과 픽셀 비교에는 Pillow를 사용합니다. HTML을 읽는 데는 Python이나 서버가 필요하지 않습니다. 소스나 실행 파일을 바꾼 뒤에는 예전 검증 결과를 새 버전의 근거로 사용하지 마세요. [컴파일러 수정 사항](verification/compiler_fixes.md)과 [제3자 권리 고지](THIRD_PARTY_NOTICES.md)도 확인하세요.
