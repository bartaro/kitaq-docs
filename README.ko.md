...
2026년 9월 14일의 소스를 기준으로 한 설명서 7권을 9개 언어로 제공합니다. 모든 언어판에는 같은 API 항목 1,053개와 완성 예제 프로그램 47개가 있습니다. 한국어는 `ko/index.html`, 영어는 `en/index.html`, 일본어는 `index.html`을 여세요. 각 권에서 언어를 바꿀 수 있습니다. 검증 기록에는 시험에 사용한 소스, 실행 파일, 조건이 명시되어 있습니다.
...
소스 수집과 빌드에는 원본 소스 트리와 각 도구가 필요합니다. 글꼴 변환과 픽셀 비교에는 Pillow를 사용합니다. HTML을 읽는 데는 Python이나 서버가 필요하지 않습니다. 검증할 때마다 사용한 소스와 실행 파일의 해시를 기록하세요. [컴파일러 수정 사항](verification/compiler_fixes.md)과 [제3자 권리 고지](THIRD_PARTY_NOTICES.md)도 확인하세요.
