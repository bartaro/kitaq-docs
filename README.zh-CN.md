...
七册正文提供九种语言版本，源码说明已更新至2026年9月14日。各语言版均收录相同的1,053个API条目和47个完整示例程序。本地阅读时，简体中文请打开 `zh-CN/index.html`，英文打开 `en/index.html`，日文打开 `index.html`。每册均可切换语言。保留的历史构建和运行记录仅对应当时的版本；更新源码说明并不代表重新完成了这些测试。
...
2. 一并上传 `index.html`、七册正文、`verification.html`、语言目录、`assets`、`samples`、`reference`、`verification`、README和许可声明。请包含 `.nojekyll`。
...
源码收集和构建需要原始源码树及相应工具。字体转换和像素比较使用Pillow。阅读HTML不需要Python或服务器。修改源码或可执行文件后，不要把旧验证结果作为新版本的证明。另请参阅[编译器修复记录](verification/compiler_fixes.md)和[第三方声明](THIRD_PARTY_NOTICES.md)。
